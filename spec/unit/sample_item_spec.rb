...
describe 'SampleItem' do
  let(:header) { ["name", "description", "price", "availability_date", "slug", "stock_total", "category"] }
  let(:row) do
    [
      "Ruby on Rails Bag", # name
  ...
  it "creates a correct SampleData with all relevant accessors" do
    ...
    expect(sample_item["name"]).to eq sample_item.data["name"]
